...
func TestBadParse(t *testing.T) {
	tests := []struct {
		...
		{`prs://admin@host/catalogname`, ErrInvalidQuery},
		{`prestos://admin@host/catalogname`, ErrInvalidQuery},
		{`prestodbs://admin:pass@host:9998/catalogname`, ErrInvalidQuery},
		{`prestos://host/cat?ssl_skip_verify=false`, ErrInvalidQuery},
...
func TestParse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			`presto://host:8001/`,
			`presto`,
			`presto://user@host:8001/default`,
			``,
		},
		{
			`presto://host`,
			`presto`,
			`presto://user@host:8080/default`,
			``,
		},
		{
			`presto://host/catalogname/schemaname`,
			`presto`,
			`presto://user@host:8080/catalogname/schemaname`,
			``,
		},
		{
			`prestodb://admin@host/catalogname`,
			`presto`,
			`presto://admin@host:8080/catalogname`,
			``,
		},
		{
			`presto://host/catalogname/schemaname?source=dburl&timezone=UTC`,
			`presto`,
			`presto://user@host:8080/catalogname/schemaname?source=dburl&timezone=UTC`,
			``,
		},
		{
			`prs://admin@host/catalogname?ssl_ca=/etc/ca.pem`,
			`presto`,
			`presto://admin@host:8443/catalogname?ssl_ca=/etc/ca.pem`,
			``,
		},
		{
			`prestodbs://admin:pass@host:9998/catalogname?ssl_skip_verify=true`,
			`presto`,
			`presto://admin:pass@host:9998/catalogname?ssl_skip_verify=true`,
			``,
		},
		{
			`presto://host/catalogname?ssl_skip_verify=false`,
			`presto`,
			`presto://user@host:8080/catalogname?ssl_skip_verify=false`,
			``,
		},
		{
			`presto://host/catalogname?ssl_cert=/etc/c.pem&ssl_key=/etc/k.pem`,
			`presto`,
			`presto://user@host:8443/catalogname?ssl_cert=/etc/c.pem&ssl_key=/etc/k.pem`,
			``,
		},
		{
			`ca://host`,
