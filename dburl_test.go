...
func TestBadParse(t *testing.T) {
	tests := []struct {
		...
		{`dm://SYSDBA:pwd@localhost/APPDB?schema=APPDB&schema=OTHER`, ErrInvalidQuery},
		{`dm://SYSDBA:pwd@localhost/APPDB?Schema=OTHER`, ErrInvalidQuery},
		{`dm://SYSDBA:pwd@localhost?schema=APPDB&Schema=OTHER`, ErrInvalidQuery},
		{`dm://SYSDBA:pwd@localhost/APPDB?schema=APPDB;rwSeparate=1`, ErrInvalidQuery},
		{`dm://SYSDBA:pwd@localhost/APPDB#OTHER`, ErrInvalidQuery},
		{`dm://SYSDBA:pwd@localhost/APPDB?=value`, ErrInvalidQuery},
...
func TestParse(t *testing.T) {
	...
	tests := []struct {
		...
		{
			`dm://SYSDBA:p%40ss%3Aword@localhost/APPDB`,
			`dm`,
			`dm://SYSDBA:p@ss:word@localhost:5236?schema=APPDB`,
			``,
		},
		{
			`dameng://SYSDBA:pwd@[::1]/APPDB?schema=appdb`,
			`dm`,
			`dm://SYSDBA:pwd@[::1]:5236?schema=appdb`,
			``,
		},
		{
			`dm8://SYSDBA:pwd@localhost/APPDB?rwSeparate=1&sslmode=verify-full`,
			`dm`,
			`dm://SYSDBA:pwd@localhost:5236?rwSeparate=1&schema=APPDB&sslmode=verify-full`,
			``,
		},
		{
			`my:`,
